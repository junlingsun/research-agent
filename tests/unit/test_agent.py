...
class TestRouteAfterEvaluation:
    ...
    def test_approved_when_score_exactly_threshold(self):
        ...
        assert (
            route_after_evaluation(self._state(score=0.7, approved=False)) == "approved"
        )
    ...
    def test_approved_at_circuit_breaker(self):
        ...
        state = self._state(
            score=0.5, approved=False, iteration_count=3, depth="standard"
        )
    ...
    def test_score_checked_before_circuit_breaker(self):
        ...
        state = self._state(
            score=0.8, approved=True, iteration_count=3, depth="standard"
        )
        assert route_after_evaluation(state) == "approved"
